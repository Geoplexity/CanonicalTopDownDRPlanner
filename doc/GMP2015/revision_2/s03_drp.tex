...
\begin{definition}\label{def:canonical_drplan}
    The \dfn{canonical DR-plan} \addin{of a graph $G$} satisfies the following three properties:
    (1) it is a DR-plan \addin{of $G$};
...
\label{theorem:main}
    A canonical DR-plan exists \addin{for a graph $G$} and any canonical DR-plan is optimal \addin{if $G$ is independent.}
\end{theorem}
...
First, note  that any  DR-plan \addin{$R$} without the Property (2) of a canonical DR-plan can always be modified (by introducing intermediate nodes) to satisfy Property (2) without  increasing the max fan-in, since any rigid proper subgraph of a graph $C$ (a child of node $C$ of the DR-plan \addin{$R$})
is the subgraph of some \addin{cluster} of $C$.
Thus without loss of generality, we can assume that \cutout{the} \addin{an} optimal DR-plan satisfies Property (2) of a canonical DR-plan.

\addin{The proof of optimality of a canonical DR-plan is by induction on its height.  The base case trivially holds for canonical DR-plans of height 0, i.e.\ for single edges. The induction hypothesis is that canonical DR-plans of height $t$ are optimal for the root node.}
\addin{For the induction step consider a canonical DR-plan $R$ of  height $t+1$ rooted at a node $C$.}
...
The first claim is that \addin{for a node $C$ whose clusters have trivial pairwise intersections}, any DR-plan \cutout{$R$} \addin{of $C$} that satisfies Property (2) must also satisfy Property (3) at $C$, i.e.\ the set of children $S$ of $C$ consists of all clusters of $C$.
\addin{Because this is the only choice, it is the minimum fan-in at $C$ for any DR-plan for $C$ with Property (2), including a known optimal one.}
The second claim shows that in the case of nodes $C$ whose rigid, vertex-maximal proper subgraphs have  non-trivial pairwise intersections, \addin{every canonical DR-plan of $C$ that uses any possible choice} of two such subgraphs of $C$ as children \cutout{ (minimum possible fan-in at $C$)} results in \addin{a minimum possible fan-in of 2 in the ancestor nodes $A$ leading to the}     {\em same maximal antichain $S$ of descendants $D$ of $C$}. \addin{The antichain is maximal in the partial order of rigid subgraphs of $C$ under containment.} I.e.\ $S$ satisfies the property that every proper vertex-maximal rigid subgraph of $C$ is a superset of some $D$ in $S$; \addin{this follows from properties of maximal antichains} that no element of $S$ is contained in the union of other elements of $S$; and the union of elements of $S$ is $C$. Thus any  DR-plan  \cutout{$R$} that satisfies Property (2) \addin{and hence contains two or more of the rigid vertex-maximal proper subgraphs of $C$ as children} must \addin{also} contain every element of $S$. The two claims complete the proof that every canonical
DR-plan is optimal.
...
\begin{remark}
The first item in the above lemma generalizes to the union of any number of children, $C_1,\ldots,C_k$, resulting in the desirable property of \dfn{cluster minimality} (defined in \cite{hoffman2001decompositionI} and in Section \ref{sec:prev}) holding for \cutout{these} canonical-optimal DR-plans.
\end{remark}
...
\begin{theorem}\label{theorem:algo_complexity}
    There exists an \candrpcomplexityv\ algorithm to find a canonical DR-plan for \addin{an input graph}.
\end{theorem}
...
In this case, an arbitrary choice of a pair of clusters as children of the node in a canonical DR-plan may not result in an optimal DR-plan if the graph is overconstrained. \todo{rephrase? Theorem 4 shows this. If it's independent it \vemph{will} be optimal (the church rosser property), no longer holds if it's overconstrained.}
A good source of examples of overconstrained graphs with canonical DR-plans that are not optimal are graphs whose cluster-minimal DR-plans that are not optimal. The example shown in Figure \ref{fig:canonicalnotoptimal} is a canonical, cluster-minimal DR-plan that is not optimal; an optimal DR-plan is also shown in the figure.
...
Similarly, the algorithm given above finds a canonical DR-plan also for any input graph.  However neither of these DR-plans may be optimal for overconstrained graphs as shown in Figure \ref{fig:canonicalnotoptimal}.
