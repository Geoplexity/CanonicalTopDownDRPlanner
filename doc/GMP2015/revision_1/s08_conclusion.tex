...
We have clarified the main source of complexity for the optimal DR-plan and recombination problems. For the former problem, when there are no overconstraints\cutout{,} (as is the case for 2D qusecs whose realizations are many common types of layered materials), we defined a canonical DR-plan and showed that \cutout{such} \addin{any} canonical DR-plan\cutout{s are} \addin{is} guaranteed to be optimal, \addin{which is} a strong Church-Rosser property\addin{. This gives} \cutout{giving} an efficient (\candrpcomplexity) algorithm to find an optimal DR-plan that satisfies other desirable characteristics.
...
\notetoreviewer{The following section was moved in from the appendix.}
...
\begin{openproblem}
    For fixed $k$ we have polynomial time optimal DR-planning \addin{(Section~\ref{sec:DRP})}, recombination (modification) in the presence of $k$ overconstraints\cutout{ (Section X}, optimal modification for decomposition {\sl OMD}$_k(G)$, when at most $k$ constraints are removed \addin{(Section~\ref{sec:recomb})} and also optimal completion using at most $k\le 2$ constraints in the body-pin and triangle-multipin cases under a modified measure for optimizing the DR-plan \addin{(Section~\ref{sec:table})}. However, \addin{in the running time of all of these algorithms,} $k$ appears in the exponent\cutout{ for the straightforward algorithms for these problems}.
\end{openproblem}
