...
\notetoreviewer{The following section was moved in from the Preliminaries section.}
...
In this paper, we will consider 3 types of constraint systems: bar-joint as well as \dfn{body-hyperpin} \addin{(defined formally in Section \ref{sec:bodypin})} and \dfn{pinned line-incidence} \addin{(defined formally in Section \ref{sec:pinnedline})}. In all of these cases, a Cartesian \dfn{realization} or \dfn{solution} $G(p)$ of $(G,\delta)$ is an assignment of coordinates or Euclidean transformations (poses), $p: V \rightarrow \mathbb{R}^2$ or $\mathbb{R}^3$, to the vertices of $G$ satisfying the constraints with parameters $\delta$, modulo orientation preserving isometries (Euclidean rigid body motions for distance constraint).
...
\cutout{In this paper, a qusecs is any independent geometric constraint system of one of the 3 types mentioned above.} \notetoreviewer{This now only in the Preliminaries section.}
