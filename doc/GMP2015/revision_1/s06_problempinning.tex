...
\notetoreviewer{Changed section title from ``Application: Finding completions of underconstrained glassy structures to isostatic''}
% \bibliography{paper}
...
In all cases, we are specifically interested in how to add additional constraints \addin{such that} the resulting isostatic structure \cutout{with} \addin{has} a small DR-plan\cutout{, so} \addin{; this way} a realization can be found\addin{, allowing efficient} \cutout{and} stress and other property design\cutout{ can be efficient}. To answer these questions, we first introduce the qusecs that are used to model these materials. We will discuss \cutout{Item 2} \addin{item (2)} in detail in this section.
...
\begin{definition}\label{def:body-pin}
    ...
    Such a body-pin graph\addin{, $G_{BP}$,} can also be seen as a \dfn{body-bar graph}\addin{, $G_{BB}$,} where the bodies \addin{of $G_{BB}$} are the original bodies \addin{of $G_{BP}$} and each pin \cutout{represents 2 bars from one body to another} \addin{between bodies in $G_{BP}$ are replaced with 2 bars in $G_{BB}$ between the same bodies}.
    Such body-bar graphs with 1 and 2-dof can be characterized by being $(3,4)$ and $(3,5)$-tight respectively \cite{Lee:2007:PGA} \cite{streinu2009sparse} (defined in \ref{sec:appendix:defs}). \addin{See Figure \ref{fig:bodypindrp:b}.}
\end{definition}

\begin{definition}
    A \dfn{triangle-hyperpin graph} is a body-hyperpin graph where each body is a triangle, i.e., it shares at most 3 pins with other bodies. This is also represented as a hyper-graph where each pin is a vertex and each triangle represents a tri-hyperedge. For such hypergraphs, 1 and 2-dof can be characterized by $(2,4)$ and $(2,5)$-tightness respectively \cite{Lee:2007:PGA} \cite{streinu2009sparse}.
\end{definition}
...
\begin{definition}
    A \dfn{$(k,l)$-tight DR-plan} is one in which each child node is either a vertex maximal proper $(k,l)$-tight sub-graph of the parent node or it is trivial. In our case, the trivial nodes are \cutout{just} the bodies.
\end{definition}
...
\begin{remark}\label{rem:1dofcanon}
    When the input is independent, all $(3,4)$-tight canonical DR-plans are optimal. We can find such a DR-plan in the same time complexity as the $(2,3)$-tight case for bar-joint graphs discussed in Section \ref{sec:DRP}. This is because such $(3,4)$-tight graphs are matroidal and have a pebble game\cutout{, shown in} \cite{Lee:2007:PGA}.
\end{remark}
...
\begin{theorem*}
\label{thm:1dofcase}
    Given a 1-dof body-pin or triangle-multipin graph \cutout{$G$} and corresponding 1-dof DR-plan \cutout{$T$}, there is a quadratic algorithm for the 1-dof optimal completion problem of Section \ref{sec:recomb}.
\end{theorem*}
