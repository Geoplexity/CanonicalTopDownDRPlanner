...
\begin{proof}
    ...
    % Suppose there were some other component $C'$ such that $C'\cap D$ was non-empty and did not equal $C'$. If the intersection were non-trivial
    % intersected f the intersection This would imply a larger isostatic graph
    If $D$ and $L$ have non-empty intersection of edge sets, $D$ cannot be exactly $L$.

    % Now observe that, given case (1), it is implied that $C$ is in fact a leaf in \branchGePG. If it were the union of several leaves, then the $f$ we chose would come from one such leaf
    ...
    % Case (2a) implies that $D$ is in $\componentsx{C\setminus f}$, to be used in the next level of recursion of the algorithm.
