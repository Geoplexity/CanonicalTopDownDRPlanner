...
The goal of this section is to develop an \ComplexityCanDRP\ time complexity algorithm for finding an optimal DR-plan.
...
\begin{proof}
    ...
    % The proofs of Claim 1 and Claim 2 prove the theorem.
\end{proof}
...
The algorithm for finding an optimal DR-plan relies on key structural properties of canonical DR-plans that are revealed by the proof of Theorem~\ref{theorem:canonical_exists_and_is_optimal}. We begin by redefining the canonical DR-plan in a recursive manner and also by recursively defining the new \dfn{\pseudosequential\ DR-plan} to highlight the similarities. We show that this \pseudosequential\ DR-plan is optimal (i.e.\ has the smallest maximum fan-in over all nodes) and has at most the same overall size (i.e.\ number of unique nodes) as a canonical DR-plan, and is in general smaller.
% By taming the large number of nodes with redundant subgraphs (in the case when the children of a node have non-trivial intersections),
...
\label{def:canonical_drp_rec}
    A \dfn{canonical DR-plan} of an isostatic $G$ is recursively defined as follows:
    \begin{enumerate}
...
\label{def:pseudoseqdrp}
    A \dfn{\pseudosequential\ DR-plan} of an isostatic $G$ is recursively defined as follows:
    \begin{enumerate}
        ...
        \item In case there are two proper vertex-maximal rigid subgraphs $C_i$ and $C_j$ of $G$ with non-trivial intersection, take the children of $G$ to be the roots of \pseudosequential\ DR-plans for $C_j\setminus C_i$ (called an \dfn{appendage}), and $C_i$ (called its \dfn{partner}).
    \end{enumerate}
\end{definition}
...
% \ClearMyMinHeight
% \SetMyMinHeight{0.195}{../../img/svg/seq_proof_figs_a}
% \SetMyMinHeight{0.195}{../../img/svg/seq_proof_figs_b}
% \SetMyMinHeight{0.195}{../../img/svg/seq_proof_figs_c}
% \SetMyMinHeight{0.195}{../../img/svg/seq_proof_figs_d}
% \SetMyMinHeight{0.195}{../../img/svg/seq_proof_figs_e}
\ClearMyMinHeight
\SetMyMinHeight{0.3}{../../img/svg/seq_proof_figs_a}
\SetMyMinHeight{0.3}{../../img/svg/seq_proof_figs_b}
\SetMyMinHeight{0.3}{../../img/svg/seq_proof_figs_c}
\SetMyMinHeight{0.3}{../../img/svg/seq_proof_figs_d}
\SetMyMinHeight{0.3}{../../img/svg/seq_proof_figs_e}
...
\begin{figure*}\centering%
    ...
    % \begin{subfigure}{0.195\linewidth}\centering
    ...
    % Note that based on the choice of $e$ as the first edge, we get a \pseudosequential\ DR-plan that looks like Figure (\ref{fig:seq_proof:c}), if we had chosen an edge from appendage $A_2$ the second and third level would be flipped. \todo{Shows cases...}
    %
    ...
    %
    % Figure (\ref{fig:seq_proof:a}) depicts $\branchx{G,e,P_G}$, the black nodes are the $\componentsx{G\setminus e}$. The node labeled `Core+$A_1$+$A_2$' is a subgraph of $G$ that has 2 isostatic vertex-maximal proper subgraphs intersecting non-trivially.
    %
    % Figure (\ref{fig:seq_proof:b}) depicts $\branchx{G,f,P_G}$ of the edge labeled $f\in L_2$ (from Figure (\ref{fig:seq_proof:a})). Similarly, the black nodes are the $\componentsx{G\setminus f}$. These leaves are labeled with the cases discussed in the proof of Lemma~\ref{lem:branch-from-branch-leaves}. \todo{emphasize in beginning.}
    %
    % Figure (\ref{fig:seq_proof:c}) depicts $\branchx{G,f,P_G}$ of the edge labeled $f$ in the core in $L_1$ (from Figure (\ref{fig:seq_proof:a})). Note that based on the choice of $e$ as the first edge, we get a \pseudosequential\ DR-plan that looks like Figure (\ref{fig:seq_proof:c}), if we had chosen an edge from appendage $A_2$ the second and third level would be flipped.
    %
    % Figure (\ref{fig:seq_proof:d}) depicts $\branchx{G,f,P_G}$ of the edge labeled $f$ in the appendage $A_1$ in $L_1$ (from Figure (\ref{fig:seq_proof:a})). Note that this contains a node $D$ that has Case 3. With this $D$, $D\cup L_1$ is `Core+$A_1$+$A_2$' and $D\cap L_1$ is `Core'. The leaves labeled $L_1'$ in Figure (\ref{fig:seq_proof:a}) are the leaves that will allow us to find the node on the path from $G$ to $e$ that is their sibling (the parent of $e$).
    %
    % Take edge $a\in G$, the $\componentsx{G\setminus a}$ is the set of nodes marked with an `x' in figure xx. The entire figure shows $\branchx{G,a,P_G}$, where $P_G$ is the \pseudosequential\ DR-plan of $G$, however, we do not yet know the contents of the `circle' nodes or the order of the nodes in the \branch.
    %
...
%     Now observe that for a leaf $L$ and edge $f\in L$, there exists component $D\in \componentsx{G\setminus f}$ can be classified as one of the following cases.
...
%     Case (1):
%     $D$ contains some components $C$ of $\componentsx{G\setminus e}$ (i.e.\ leaves of \branchGePG) and intersects all other leaves of \branchGePG\ trivially. One occurrence of this case (Case 1a) is when $L$'s siblings in (any \pseudosequential\ DR-plan) $P_G$ have trivial pairwise intersection. Another occurrence of this case (Case 1b) is when $L$'s siblings in (any \pseudosequential\ DR-plan) $P_G$ have non-trivial pairwise intersections, but the edge $f$ is in their common intersection, i.e.\ the core \todo{mentions core!}. In both cases, $D$ is a node along the path from $e$ to $G$ in $P_G$, with the components $C$ being its descendants, $L$ being its sibling and all other components in $\componentsx{G\setminus e}$ (i.e.\ leaves of \branchGePG) being children of $D$'s ancestors in \branchGePG.
%     \todo{(1c), it contains no components.}


%     Case (2):
%     $D$ is contained in $L$ and intersects all other leaves of \branchGePG\ trivially. In this case, $D$ is a leaf of \branchLfPL, to be used in the next level recursion of the algorithm.
...
\begin{proof}
    ...
    % Since each level of \branchGePG\ will contain some $L$ and $f\in L$ with a $D\in \componentsx{G\setminus f}$ that is either of Case (1a), (1c), or (2b) all nodes in the path from $G$ to $e$ can be found, which completes the proof.
...
The class of sequential DR-plans is defined as a subset of \pseudosequential\ and satisfies all properties discussed in the previous section. Furthermore, we show that a sequential DR-plan can always be obtained from a \pseudosequential\ DR-plan. A sequential DR-plan is desirable because it is `essentially' unique, and illustrates that the canonical DR-plan in fact retains the essential uniqueness of a complete DR-plan.
 % which will be elaborated upon following the definition.

\begin{definition}
[Sequential DR-plan, core]
\label{def:seqdrp}
    A \dfn{sequential DR-plan} is a \pseudosequential\ DR-plan that additionally satisfies the following requirement:
...
%     \caption{Two sequential DR-plans of the same isostatic input graph. \todo{More caption. Show \pseudosequential\ that isn't sequential.}}
%     \label{fig:seqdrp_nonuniqueness}
% \end{figure}


\ClearMyMinHeight
\SetMyMinHeight{.3}{../../img/svg/pseudoseq_vs_seq_seq}
\SetMyMinHeight{.3}{../../img/svg/pseudoseq_vs_seq_pseudoseq}
...
\ClearMyMinHeight
\SetMyMinHeight{.6}{../../img/svg/new2_overconstrained_not_optimal}
\SetMyMinHeight{.4}{../../img/svg/new2_overconstrained_optimal}
