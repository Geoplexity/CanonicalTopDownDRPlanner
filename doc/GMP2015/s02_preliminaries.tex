...
\noindent {\bf Note:} Geometric constraint systems can also have inequalities, 
...
%implies rigidity)
%When the rigidity matrix has appropriate rank, the realizations or solutions 
%of the corresponding constraint system are generically isolated and zero-dimensional 
...
%this claim is approximate: the solutions are isolated small, full-dimensional  connected components.

One important property of generic constraint system or 
(hyper)graph %that depends on the appropriate rigidity matrix of indeterminates
...
\noindent {\bf Note:} If the constraint system has inequalities, i.e, if the $\delta$ lie in an interval,
...
Other generic constraint system or (hyper)graph properties are the following: %({\bf Note:} we refer to these as
%properties of the constraint system or as properties of the underlying
%(hyper)graph interchangeably).
%
...
Defining the independence of a subset of edges $E' \subseteq E$ 
...
%TODO
Given a bar-joint graph, a vector (of indeterminates) in the right null space of its rigidity matrix
...
A {\em trivial} graph is   a single vertex.
