...
% %
%     A DR-plan is \dfn{complete} if it satisfies an additional property: for a non-leaf node $C$, its children are all of the rigid vertex-maximal proper subgraphs of $C$. This makes Property 2 implicit.
...
% We begin with the definition of a general DR-plan and also introduce two specific kinds of DR-plans.
...
\begin{definition}
    A DR-plan is \dfn{complete} if it satisfies an additional property: for a non-leaf node $C$, its children are all of the rigid vertex-maximal proper subgraphs of $C$ (which makes Property 3 of a DR-plan implicit.)
\end{definition}

\begin{definition}
    A DR-plan is \dfn{optimal} if it minimizes the maximum fan-in over all nodes in the forest. The maximum fan-in is called the \dfn{size} of the DR-plan.
\end{definition}
