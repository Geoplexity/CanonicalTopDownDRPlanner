...
_DEPS = Graph/Graph.hpp Graph/DR_Plan.hpp \
	...
	GUI/Main_GUI_Manager.hpp GUI/Camera_2D.hpp GUI/Shader_Program.hpp GUI/gl_objects.hpp GUI/easy_font.hpp GUI/Drawer.hpp \
...
$(OBJECT_DIRECTORY)/%.o: $(SOURCE_DIRECTORY)/%.cpp $(DEPS)
	...
	# $(CC) -g -c -o $@ $< $(CFLAGS) $(INCLUDE_PATHS) $(LIBRARY_PATHS) $(LINKER_FLAGS)

$(EXEC): $(OBJ)
	$(CC) -o $@ $^ $(CFLAGS) $(INCLUDE_PATHS) $(LIBRARY_PATHS) $(LINKER_FLAGS)
	# $(CC) -g -o $@ $^ $(CFLAGS) $(INCLUDE_PATHS) $(LIBRARY_PATHS) $(LINKER_FLAGS)
