    ...
    float height_of_letters)
{
    ...
    for (i=0; i < num_segs; ++i) {
        ...
        if (len) {
            ...
            for (j=0; j < 4; ++j) {
                gl_obj::pos_vec pos;
                pos[0] =  (x  + (j==1 || j==2 ? (vertical ? 1 : len) : 0))*height_of_letters/7;
                pos[1] = -(y0 + (    j >= 2   ? (vertical ? len : 1) : 0))*height_of_letters/7;

                gl_obj::Vertex v(pos, color);

                vg->push_back(v);
            }

            vg->push_back(vg->at(vg->size()-2));
            vg->push_back(vg->at(vg->size()-5));
        }
    }
}
    ...
    float spacing = 0)
{
    ...
    for (int i = 0; text[i]; i++) {
        if (text[i] == '\n') {
            // y += 12;
            // y += 7 + 5 space_between_lines;
            y += 7 + (7 * (space_between_lines/height_of_letters));
            x = start_x;
        } else {
            unsigned char advance = stb_easy_font_charinfo[text[i]-32].advance;
            float y_ch = advance & 16 ? y+1 : y;
            int h_seg, v_seg, num_h, num_v;
            h_seg = stb_easy_font_charinfo[text[i]-32  ].h_seg;
            v_seg = stb_easy_font_charinfo[text[i]-32  ].v_seg;
            num_h = stb_easy_font_charinfo[text[i]-32+1].h_seg - h_seg;
            num_v = stb_easy_font_charinfo[text[i]-32+1].v_seg - v_seg;
            stb_easy_font_draw_segs(&vg, x, y_ch, &stb_easy_font_hseg[h_seg], num_h, 0, color, height_of_letters);
            stb_easy_font_draw_segs(&vg, x, y_ch, &stb_easy_font_vseg[v_seg], num_v, 1, color, height_of_letters);
            x += advance & 15;
            x += spacing;
        }
    }
